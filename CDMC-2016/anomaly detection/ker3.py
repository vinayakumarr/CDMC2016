...
#testdata = pd.read_csv('normalized/anamoly_train.csv', header=None)
...
print(model.summary())
model.fit(X_train, y_train, batch_size, nb_epoch=1, validation_data=(X_test, y_test))
loss, accuracy = model.evaluate(X_test, y_test)
print("\nLoss: %.2f, Accuracy: %.2f%%" % (loss, accuracy*100))


y_pred = model.predict_classes(X_test)
np.savetxt('predictedlabelscnnlstm.txt', np.transpose([y_test1,y_pred]), fmt='%01d')
